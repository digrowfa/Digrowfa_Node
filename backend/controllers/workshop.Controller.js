...
export const getWorkShops = async (req, res) => {
    try {
        ...
        let courses = await Workshop.find()
            .populate([
                { path: "category", select: "name" },
                { path: "language", select: "name" }
            ])
        return res.status(200).json({ errorcode: 0, status: true, msg: "WorkShops Found.", data: courses });
    } catch (e) {
        return res.status(200).json({ errorcode: 5, status: false, msg: e.message, data: e });
    }
};

export const getEnrolledWorkShops = async (req, res) => {
    ...
    try {
        ...
        const {id}=req.params;
        let userData = await User.findById(id).populate([{path:"batches",populate:{path:"workshop",model:"Workshop"}}])
           
        return res.status(200).json({ errorcode: 0, status: true, msg: "workShops Found Found.", data: userData });
    } catch (e) {
        return res.status(200).json({ errorcode: 5, status: false, msg: e.message, data: e });
    }
};

export const getWorkshopdetails = async (req, res) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            let errs = [];
        ...
        const { id } = req.params
        let course = await Workshop.findById(id).populate([
            { path: "category", select: "name" },
            { path: "language", select: "name" }
        ])
        if (!course) return res.status(200).json({ errorcode: 2, status: true, msg: "Workshop Not Found.", data: null });
        return res.status(200).json({ errorcode: 0, status: true, msg: "Workshop Found.", data: course });
    } catch (e) {
        return res.status(200).json({ errorcode: 5, status: false, msg: e.message, data: e });
    }
};
...
export const createWorkshop = async (req, res) => {
    ...
    try {
       ...
       console.log(people);
        ...
        return res.status(200).json({ errorcode: 0, status: true, msg: "Workshop Created Succesffully.", data: course });
    } catch (e) {
        console.log(e)
        return res.status(200).json({ errorcode: 5, status: false, msg: e.message, data: e });
    }
...
export const editWorkshop = async (req, res) => {
    ...
    try {
        ...
        let course = await Workshop.findById(id);
        ...
        const a=await Workshop.findOneAndUpdate({_id:id},{$set:{"bonuses.bonus":bonus}})
        console.log("a",a);
        return res.status(200).json({ errorcode: 0, status: true, msg: "Workshop Updated Succesffully.", data: course });
    } catch (e) {
        console.log(e)
        return res.status(200).json({ errorcode: 5, status: false, msg: e.message, data: e });
    }
...
export const editWorkshopFaq = async (req, res) => {
    console.log("editCourse ")
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
        ...
        let course = await Workshop.findById(id);
        if (!course) return res.status(200).json({ errorcode: 3, status: false, msg: "Workshop is Not Present", data: null })
        course.faq=people
        course = await course.save()
...
export const deleteWorkshop = async (req, res) => {
    try {
        ...
        const { id } = req.params
        if (!id) return res.status(200).json({ errorcode: 2, status: true, msg: "Workshop Id is Required.", data: null });
        let courses = await Workshop.findById(req.params.id)
        if (!courses) return res.status(200).json({ errorcode: 2, status: true, msg: "Workshop Not Found.", data: null });
        await Workshop.deleteOne({ _id: req.params.id })
        return res.status(200).json({ errorcode: 0, status: true, msg: "Workshop deleted Successfully.", data: null });
    } catch (e) {
        return res.status(200).json({ errorcode: 5, status: false, msg: e.message, data: e });
    }
};
...
export const createLesson = async (req, res) => {
    try {
        ...
        let courseFound = await Workshop.findById(course_id)
        if (!courseFound) return res.status(200).json({ errorcode: 3, status: false, msg: "No Workshop Found", data: null });
        ...
        let course = await Workshop.updateOne({ _id: course_id }, { $push: { lessons: lesson } }, { new: true });
...
export const editLesson = async (req, res) => {
    ...
    try {
        ...
        let lessondata = await Workshop.findOne({ lessons: { $elemMatch: { _id: id } } })

        let lessondatas = await Workshop.updateOne(
            { "lessons._id": id },
            {
                $set: {
                    "lessons.$.title": title,
...
export const deleteLesson = async (req, res) => {
    try {
        ...
        if (!course || !lesson) return res.status(200).json({ errorcode: 2, status: false, msg: "course and lesson ids are needed", data: null });
        let lessondata = await Workshop.updateOne({ _id: course }, { $pull: { lessons: { _id: lesson } } })
...
export const updateLessonArrangmaent = async (req, res) => {
    try {
        ...
        if (!courseFound) return res.status(200).json({ errorcode: 2, status: false, msg: "No course Found", data: null });
        let course = await Workshop.findOneAndUpdate({ _id: course_id }, req.body, { new: true });
...
export const getCategoryWorkshop = async (req, res) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            let errs = [];
        ...
        const { id } = req.params
        let courses = await Workshop.find({ category: id })
            .populate([
                { path: "category", select: "name" },
                { path: "language", select: "name" }
            ])
        return res.status(200).json({ errorcode: 0, status: true, msg: "Workshop Found.", data: courses });
    } catch (e) {
        return res.status(200).json({ errorcode: 5, status: false, msg: e.message, data: e });
    }
};
